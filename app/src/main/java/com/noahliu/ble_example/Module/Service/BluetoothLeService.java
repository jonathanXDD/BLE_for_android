...
public class BluetoothLeService extends Service {
    ...
    public boolean initialize() {
        try {
            if (mBluetoothManager == null) {
                Log.d(TAG, " Context.BLUETOOTH_SERVICE.." + Context.BLUETOOTH_SERVICE);
                Object a = getSystemService(Context.BLUETOOTH_SERVICE);
                if (a == null) {
                    return false;
                }
                mBluetoothManager = (BluetoothManager) a;
            }
    ...
    public boolean connect(final String address) {
        Log.d(TAG, "connect.start.."+  address);
        if (mBluetoothAdapter == null || address == null) {
            Log.d(TAG, "connect.mBluetoothAdapter is null"+  mBluetoothAdapter +address );
            return false;
        }

        if (address.equals(mBluetoothDeviceAddress) && mBluetoothGatt != null) {
            Log.d(TAG, "connect...1.0" );
            if (mBluetoothGatt.connect()) {
                mConnectionState = STATE_CONNECTING;
                Log.d(TAG, "connect...1.1" );
                return true;
            } else {
                Log.d(TAG, "connect..1.2" );
                return false;
            }
        }
        Log.d(TAG, "connect..2.0" );
        ...
        mConnectionState = STATE_CONNECTING;
        Log.d(TAG, "connect..4 end.mConnectionState:" + mConnectionState );
        return true;
    }
    /**斷開連線*/
    ...
    private void setCharacteristicNotification(BluetoothGattCharacteristic characteristic, boolean enabled) {
        Log.d(TAG, "setCharacteristicNotification star");
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            return;
        }
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            Log.d(TAG, "onConnectionStateChange start.status."+ status + " newState:"+newState);
            ...
            Log.d(TAG, "onConnectionStateChange end");
        }
